export default class Coin {
    ...
    randomPosition10() {
        let randomInt = () => {
           return Math.floor(Math.random() * (this.gameHeight/10 - this.radius/10 + 1) + this.radius/10)
        }
        return randomInt() * 10
    ...
    update(deltaTime) {
        if (this.position.x > this.snek.position.x &&
        ...
        console.log(this.position.x, Math.floor(this.snek.width/2))
    }
}